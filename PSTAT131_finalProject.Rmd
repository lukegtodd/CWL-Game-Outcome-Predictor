...
all_hp_2019 <- sqldf('SELECT player, win, k_d, assists, damage_dealt, player_spm, hill_time_s, hill_captures, hill_defends FROM allMajors2019 WHERE mode == "Hardpoint"')
...
playerWinsHP <- playerWinsHP %>% count(player) # counts the number of wins per player

all_hp_2019_mb <- all_hp_2019_mb %>%
  group_by(player) %>%
  ...
  subset(., select = -c(k_d, assists, damage_dealt, hill_time_s, hill_captures, hill_defends, player_spm, win)) %>%
  unique()
...
all_hp_2019_mb <- sqldf('SELECT * FROM all_hp_2019_mb INNER JOIN playerWinsHP ON all_hp_2019_mb.player = playerWinsHP.player')
...
# pairs(win ~ k_d + player_spm + damage_dealt + assists + hill_time_s + hill_captures + hill_defends, data = all_hp_2019)

mydata.cor = cor(all_hp_2019, method = c("spearman"))
