...
hp_test_recipe <- recipe(win ~ k_d + assists + damage_dealt + 
...
## Model 2: Random Forest TEST TEST TEST
...
autoplot(hp_rf_tune, metric = "accuracy")
...
augment(hp_class_tree_final_fit, new_data = hp2019_train) %>%
  accuracy(truth = win, estimate = .pred_class)
...
hp_rf <- randomForest(win ~ k_d + role + assists + damage_dealt + 
                        player_spm + hill_time_s + hill_captures + 
                        hill_defends + x2_piece + x3_piece + x4_piece, 
                      data = hp2019_train, 
                      ntree = 500)
...
hp_rfAccuracy <- (1304 + 1313) / (1304 + 1313 + 467 + 467)
hp_rfAccuracy
...
hp_recipe <- recipe(win ~ k_d + role + assists + damage_dealt + 
                        player_spm + hill_time_s + hill_captures + 
                        hill_defends + x2_piece + x3_piece + x4_piece, 
                      data = hp2019_train)
