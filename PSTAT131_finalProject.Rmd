...
###**RE-DOING DATA CLEANING TO INCLUDE EVERY PLAYER FROM THE TEAM ONE OBSERVATION**

```{r}

# getting all necessary data for hardpoint
mergedhp2019 <- sqldf('SELECT match_id, team, player, kills, deaths, win, assists, damage_dealt, player_spm, hill_captures, hill_defends FROM majors2019 WHERE mode == "Hardpoint"')
...
test_mergedhp2019$win <- ifelse(test_mergedhp2019$win == "W", 1, 0) %>%
  as.factor()
...
**Now let's try a different approach, essentially from scratch. Let's try to predict a win (YES) or a loss (NO) using a player's data. DECISION TREE METHOD** Most likely, I should do this by gamemode. Otherwise, certain columns will be empty since some of the data is not relevant in different gamemodes.
...
Checking confusion matrix and accuracy of the test data:
```{r}

augment(class_tree_fit, new_data = hp2019_test) %>%
  conf_mat(truth = win, estimate = .pred_class)
...
Checking confusion matrix and accuracy of the Champs data:
```{r}

augment(class_tree_fit, new_data = hpChamps) %>%
  conf_mat(truth = win, estimate = .pred_class)
...
BASED ON HW'S. NEEDS A LOT OF WORK
...
bagging_fit <- fit(bagging_spec, win ~ k_d + assists + damage_dealt + player_spm + hill_time_s + hill_captures + hill_defends, data = hp2019_train)
...
augment(bagging_fit, new_data = hp2019_test) %>%
  rmse(truth = win, estimate = .pred)
...
rf_fit <- fit(rf_spec, win ~ k_d + assists + damage_dealt + player_spm + hill_time_s + hill_captures + hill_defends, data = hp2019_train)

```

```{r}

augment(rf_fit, new_data = hp2019_train) %>%
  rmse(truth = win, estimate = k_d)
...
# TRYING DECISION TREE ON MERGED TEAM DATA
...
mergedhp <- team_mergedhp2019 %>%
  initial_split(prop = 0.8, strata = "win")

mergedhp_train <- training(mergedhp)
mergedhp_test <- testing(mergedhp)
...
prop.table(table(mergedhp_train$win))
...
augment(class_tree_fit, new_data = mergedhp_train) %>%
  conf_mat(truth = win, estimate = .pred_class)
...
augment(class_tree_fit, new_data = mergedhp_test) %>%
  conf_mat(truth = win, estimate = .pred_class)
