...
allMajors2019 <- sqldf('SELECT * FROM allMajors2019 WHERE damage_dealt != "0"')

# # converts win to numeric where 1 is Loss and 2 is Win
# allMajors2019$win <- factor(allMajors2019$win, levels = as.character(unique(allMajors2019$win)))
# allMajors2019$win <- as.numeric(allMajors2019$win)
...
all_hp_2019 <- sqldf('SELECT player, k_d, win, kills, deaths, x, assists, damage_dealt, player_spm, hill_time_s, hill_captures, hill_defends FROM allMajors2019 WHERE mode == "Hardpoint"')
...
# all 2019 SND data
all_snd_2019 <- sqldf('SELECT player, win, k_d, assists, damage_dealt, player_spm, bomb_sneak_defuses, bomb_plants, bomb_defuses, snd_rounds, snd_firstbloods FROM allMajors2019 WHERE mode == "Search & Destroy"')
...
all_control_2019 <- sqldf('SELECT player, win, k_d, assists, damage_dealt, player_spm FROM allMajors2019 WHERE mode == "Control"')
...
allMajors2019 <- sqldf('SELECT * FROM allMajors2019 WHERE damage_dealt != "0"')
playerDamage <- sqldf('SELECT player, damage_dealt FROM allMajors2019 WHERE damage_dealt != "0"')
...
playerWinsHP <- sqldf('SELECT player, win FROM all_hp_2019_mb WHERE win == "W"') # selects all the wins for each player
playerWinsHP <- playerWinsHP %>% count(player) # counts wins per players
...
playerGamesPlayed <- sqldf('SELECT player, win FROM all_hp_2019_mb')
playerGamesPlayed <- playerGamesPlayed %>% count(player) # calculates the number of games played per player

playerWinRateHP <- merge(playerWinsHP, playerGamesPlayed, by = c("player")) %>%
  mutate(winRate = n.x/n.y) %>%
  subset(., select = -c(n.x, n.y))

all_hp_2019_averages <- all_hp_2019_mb %>%
  group_by(player) %>%
...
all_hp_2019_averages <- merge(playerWinRateHP, all_hp_2019_averages, by = c("player"))
...
all_hp_2019_totals <- all_hp_2019_mb %>%
  group_by(player) %>%
...
# creates a final dataset for hardpoint where each player only has one row, which includes the averages of all of his statistics
all_hp_2019_totals <- merge(playerWinsHP, all_hp_2019_totals, by = c("player"))
...
pairs(winRate ~ avg_kd + avg_spm + avg_damagedealt + avg_assists + avg_hilltime + avg_captures + avg_defends, data = all_hp_2019_averages)

corr_averages_hp_2019 <- all_hp_2019_averages %>%
...
winRate <- lm(winRate ~ avg_kd + avg_spm + avg_assists + avg_damagedealt + avg_hilltime + avg_captures + avg_defends, data = all_hp_2019_averages)
summary(wins)
...
plot(all_hp_2019_averages$winRate, all_hp_2019_averages$avg_kd, xlab = "Win Rate", ylab = "Average K/D",
...
pairs(n ~ tot_kills + tot_deaths + tot_spm + tot_assists + tot_damagedealt + tot_hilltime + tot_captures + tot_defends, data = all_hp_2019_totals)

corr_totals_hp_2019 <- all_hp_2019_totals %>%
  subset(., select = -c(player))

cor(corr_totals_hp_2019)
...
plot(all_hp_2019_totals$n, all_hp_2019_totals$tot_x, xlab = "Number of Wins", ylab = "Total Kills - Total Deaths",
...
winsTotal <- lm(n ~ tot_kills + tot_deaths + tot_spm + tot_assists + tot_damagedealt + tot_hilltime + tot_captures + tot_defends, data = all_hp_2019_totals)
summary(wins)
...
all_hp_2019_wl <- all_hp_2019

# changes W to 1, L to 0
# all_hp_2019_wl$win <- ifelse(all_hp_2019_wl$win == "W", 1, 0) %>%
#   as.factor()

all_hp_2019_wl$win <- as.factor(all_hp_2019_wl$win)
...
prop.table(table(all_hp_2019_wl$win))
...
fit <- rpart(win ~ k_d + assists + damage_dealt + player_spm + hill_time_s + hill_captures + hill_defends, data = all_hp_2019_wl, method = 'class')
...
predict_unseen <- predict(fit, champs2019, type = 'class')

table_mat <- table(champs2019$win, predict_unseen)
table_mat

```

The model correctly predicted 1168 losses, but predicted 311 losses as wins. In addition, the model correctly predicted 1024 wins, but predicted 456 wins as losses.
