...
# whole event data, organized
champs2017 <- champs2017[order(champs2017$player),] # organizes all the players in alphabetical order
...
# all player k_d card
allPlayers_champs2017 <- sqldf('SELECT player, k_d FROM champs2017')

# sample boxplot comparing the KD of each player that played in the event
ggplot(test, aes(x = player, y = k_d)) + geom_boxplot() + coord_flip()
